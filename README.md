...
is_fizzbuzz = lambda x: is_fizz(x) and is_buzz(x)
fizzbuzz = lambda x: "FizzBuzz"

# Create Pipe and make FizzBuzz using single functions and fork
p = Pipe()
p_fizzbuzz = p.fork(is_fizzbuzz, fizzbuzz, is_buzz, buzz, is_fizz, fizz, str)
...
Forking simulates a conditional structure like if, elif, and else. It acts like a combination of `filter()` and `through()`. It will execute a given action if a given condition is true. Otherwise it will pass the value unchanged to either the next condition and action or the end of the operation. This enables branching of the Stream into more of a DAG than a single sequence of operations.

This operation can be handled with *n* arguments (*condition1, argument1, condition2, argument2, . . . conditionN, argumentN*), where *n* is even, where if a condition is met then the value has the following argument evaluated on it. If no conditions are met then the value it passed through unchanged. If *n* is odd then if no conditions are met than the final argument is executed on the value and returned.
```python
from PyStream.stream import Stream, Pipe
...
# Stream the values through a fork that multiplies evens by -1 then adds 10 to all.
